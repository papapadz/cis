    ...
    <div class="row">
      <div id="divcontent" style="font-size:10px; background-color: #f8f7f7; border-radius:2px; padding: 20px; border:1px solid #ccc; margin-bottom: 20px;  " >
        ...
        <div class="pull-right" align="right" style="padding-right:0px;" >
            <a href="{{url('admin/viewaddworkexp')}}" >
          ...
          <div class="table-responsive" >
            <table class="table table-bordered">
                <tr>
                  ...
                  <th class="info">Goverment</th>
                  <th class="info">Action</th>
                </tr>
                <tbody id="tbodworkexp" >
                  @forelse($arrworkexp as $workexp)
                    <tr>
                      <td>{{$workexp->work_position}}</td>
                      <td>{{$workexp->work_status}}</td>
                      <td>{{$workexp->company_name}}</td>
                      <td>{{$workexp->company_address }}</td>
                      <td>{{number_format($workexp->salary,2,'.',',')}}</td>
                      <td>{{$workexp->start_date}}</td>
                      <td>{{$workexp->end_date}}</td>
                      <td>
                        @if($workexp->is_government == 'Y')
                          Yes
                        @elseif($workexp->is_government == 'N')
                          No
                        @endif
                      </td>
                      <td>
                        <a class="edit" href="{{url('admin/vieweditworkexp/'.$workexp->work_exp_id)}}">[ edit ]</a>
                        <a class="delete" style="color:red;" data-depid="{{$workexp->work_exp_id}}" >[ delete ]</a>
                      </td>
                    </tr>
                  @empty
                    <tr>
